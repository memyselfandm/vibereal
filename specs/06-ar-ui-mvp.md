...
### Unity app
1. Open `unity/` as a Unity project (Unity 2022.3 LTS+)
2. Unity will auto-resolve packages from `Packages/manifest.json` (NativeWebSocket, TextMeshPro, etc.)
3. Import TMP essentials when prompted (Window → TextMeshPro → Import TMP Essential Resources)
4. **Build the scene automatically:** Menu → VibeReal → Build MVP Scene
   - This editor script creates all GameObjects, UI elements, and wires all SerializeField references
5. Save the scene as `Assets/Scenes/MainScene.unity`
6. Select the WebSocketClient in the Managers object and set `hubUrl` to `ws://<your-machine-ip>:8080`
7. Enter Play mode

### Building for Beam Pro
1. File → Build Settings → Switch Platform to Android
2. Player Settings are pre-configured (ARM64, IL2CPP, API 29+, OpenGLES3)
3. AndroidManifest.xml is included at `Assets/Plugins/Android/`
4. Install XREAL SDK 3.1.0:
   - Download `com.xreal.xr-3.1.0.tgz` from [developer.xreal.com/download](https://developer.xreal.com/download/)
   - Window → Package Manager → + → Add package from tarball → select the `.tgz`
   - Edit → Project Settings → XR Plug-in Management → Android tab → enable **XREAL**
   - Import XR Interaction Toolkit "Starter Assets" from the Samples tab
5. Build APK: File → Build Settings → Build
6. Sideload: `adb install VibeReal.apk`

### Scene Hierarchy (auto-generated by VibeReal > Build MVP Scene)
...
├── Canvas (Screen Space Overlay — switch to World Space for AR)
│   ├── SessionPanel         [SessionPanelUI.cs]
│   │   ├── Header
│   │   │   ├── StatusDot (Image)
│   │   │   ├── SessionName (TMP_Text)
│   │   │   └── StatusLabel (TMP_Text)
│   │   ├── ConversationArea (ScrollRect + RectMask2D)
...
│   │       ├── ApprovalTitle (TMP_Text)
│   │       ├── ApprovalBody (TMP_Text)
│   │       ├── ApproveButton (Button)
│   │       └── DenyButton (Button)
│   ├── StatusBar            [StatusBarUI.cs]
│   │   ├── ConnectionDot (Image)
│   │   ├── ConnectionLabel (TMP_Text)
│   │   ├── SessionCount (TMP_Text)
│   │   └── Clock (TMP_Text)
│   └── NotificationArea     [NotificationToastUI.cs]
